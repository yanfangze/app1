...
def generate_plotly_bar_chart(word_counter):
    most_common_20 = word_counter.most_common(20)
    df = pd.DataFrame(most_common_20, columns=['Word', 'Frequency'])
...
if url:
    ...
    if text:
        ...
        chart_type = st.sidebar.selectbox("选择图表类型", ["词云", "柱状图", "折线图", "饼图", "雷达图", "散点图", "漏斗图", "柱状图 (Plotly)", "饼图 (Matplotlib)"])
        chart_generators = {
